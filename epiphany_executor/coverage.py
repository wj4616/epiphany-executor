...
# ---------------------------------------------------------------------------
# wiring-check closure gate integration (anti-false-green; Phase 1 deploy)
# ---------------------------------------------------------------------------
def coverage_report_with_closure(plan: dict, skill_pkg, *, plan_path=None) -> dict:
    """Requirement-coverage report AUGMENTED with the wiring-closure gate for a harness-skill
    build. The run reaches ``done`` only if BOTH requirement-coverage AND wiring-closure are
    green; a red wiring gate yields ``closure_blocked=True`` carrying the gap-list. This is the
    deterministic anti-false-green enforcement (see closure_gate / N-coverage_and_report §5).
    """
    from .closure_gate import coverage_closure_with_gate
    cov = coverage_report(plan)
    base = {
        "requirement_matrix": cov.matrix,
        ...
        "requirement_blocks": cov.blocks,
    }
    return coverage_closure_with_gate(plan, skill_pkg, base, plan_path=plan_path)
