...
2. **Adapt the actual emitted shape via tolerant field adapters.** Do not assume a single canonical encoding. Account for the real variants:
  ...
  - **`plan_meta.target_profile` + `plan_meta.harness_forge`** (additive; present only when the plan targets a goatcs/epiphany-harness skill — see `~/docs/epiphany/harness-forge-pipeline-integration.md`) — capture both into `plan_metadata` verbatim. Absent ⇒ `target_profile` defaults to `generic` and nothing downstream changes. Do NOT synthesize or infer them; carry through exactly what the plan declares. (The field census wires `target_profile`/`harness_forge`/`target_subsystem` to S-P2-context, so they will not trip the INV-2 unconsumed-field BLOCK.)
